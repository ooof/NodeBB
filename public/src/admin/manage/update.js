...
define('admin/manage/update', [], function () {
	...
	Update.init = function () {
		var updateV11 = $('[data-action="update-version-11"]'),
			...
			updateV17 = $('[data-action="update-version-17"]');
		...
		updateV17.on('click', function () {
			socket.emit('admin.update.version.V17', {version: 'v17'}, function (err) {
				if (err) {
					return app.alertError(err.message);
				}
				updateV17.next().text('升级完成...')
			});
		});
	};
